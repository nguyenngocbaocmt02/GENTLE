...
class ImPlotter(Plotter):
    ...
    def __init__(self, ipf, args, im_dir = './im', gif_dir='./gif'):
        ...
        self.metrics_dict = {"psnr": PSNR(data_range=255.).to(self.ipf.device),
                             "ssim": SSIM(data_range=255., compute_on_cpu=True if self.ipf.accelerator.state.distributed_type == "NO" and self.ipf.save_npar >= 1000 else False).to(self.ipf.device),
